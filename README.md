# Obsidian Better Command Palette ddootts

This is a personal fork of Better Command Palette for Obsidian, with tweaks for my own vault and workflow. It is not intended to be a general-purpose replacement for the upstream community plugin.
...
3. Built in quick switcher by typing directly or using the hotkey
...
### File Opening
Better Command Palette ddootts allows you to open files from the same input without needing to run a command or press `cmd+o` first. Once the palette is open, type a file name to search files. Type `>` to search commands. These prefixes can be changed in the settings. Press `enter` to open the file.
...
Better Command Palette ddootts allows you to find and open files that contain the tags you search for.
...
### Macro Commands
Macros can be created in the settings tab for Better Command Palette ddootts. Each Macro must be give a name, delay, and at least one command. If any of these are not set the macro will not show up in the command palette.
...
This builds the plugin in production mode and copies the needed files to the root of the repo (automatically ignored by git). This is for manually installing the personal fork into an Obsidian vault.

### Production Build
Run `npm run build`

Builds the plugin for production and puts all necessary files into the `dist` directory. GitHub Actions uses this for tagged releases from this fork.
